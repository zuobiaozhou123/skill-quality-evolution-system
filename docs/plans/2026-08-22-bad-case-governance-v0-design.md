...
## 10. V0 实现状态（2026-08-22）

当前架子已经按本设计落地：

| 层次 | 已实现能力 |
|---|---|
| 观察层 | 只读索引最近 Codex JSONL 会话，提取任务摘要、工作目录、Skill 加载与候选异常信号 |
| 治理层 | Bad Case 创建、编辑、确认、驳回、归因和 Evidence 资产化状态机 |
| 资产层 | 扫描本机 Skill、生成内容指纹，并支持显式登记治理对象 |
| 控制层 | 变更提案和发布中心保留真实空状态，不生成候选、不执行发布 |
| 存储层 | SQLite 保存本地运行状态；登记信息和 Evidence 使用可审查 JSON 文件 |
| 交互层 | React + Ant Design 本地工作台，七个页面连接真实本地 API |

状态迁移具有独立的确认、驳回、归因和资产化时间。确认后的事实若被再次编辑，会自动退回待确认；归因后不再允许改写。可提交 Registry 只保存相对 Skill 路径，Evidence 不保存原始任务摘要、会话 ID 或会话路径。本地 API 不向其他网站开放跨域访问。

当前运行边界：系统只观察已有运行，并等待用户主动标记 Bad Case；不会批量创建案例，不会修改正式 Skill，也不会自行调用模型。为控制隐私暴露，V0 暂不提供原始会话片段查看；独立人工新建入口和总览中的登记 Skill 明细也尚未启用，完整清单统一在 Skill 资产页查看。下一阶段应先选择首批试点 Skill 和成功标准，再启用候选生成与评测能力。

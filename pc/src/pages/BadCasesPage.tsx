...
import type { AttributionType, BadCase, BadCaseStatus } from "../types";
...
export function BadCasesPage() {
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    const text = `${item.title} ${item.problem} ${item.skillNames.join(" ")}`.toLowerCase();
  ...
  return (
    <>
      ...
      <section className="content-section flush-section">
        <Table
          columns={[
            ...
            { title: "操作", key: "action", width: 100, render: (_: unknown, item: BadCase) => <Button icon={<Eye size={15} />} onClick={() => setSelected(item)}>查看</Button> },
          ]}
          dataSource={filtered}
          loading={loading}
      ...
      <Drawer destroyOnHidden onClose={() => setSelected(undefined)} open={Boolean(selected)} title="Bad Case 详情" width={540}>
        {selected && (
          <BadCaseDetail
            item={selected}
